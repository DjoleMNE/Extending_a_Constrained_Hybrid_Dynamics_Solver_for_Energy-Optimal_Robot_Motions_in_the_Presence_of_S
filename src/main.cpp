...
class Friction_enabled_vereshchagin
{
    ...
    int numb_joints = 0, numb_segments = 0;
    ...
    void iterate_over_torques(const std::vector<double> resolution, int joint, const std::vector<int> steps_set, std::vector<double> resulting_set){
        ...
        if (joint >= numb_joints) {
            ...
            KDL::JntArray* temp_torques = new KDL::JntArray(numb_joints);

            for(int j = 0; j < numb_joints; j++){
                temp_torques[0](j) = resulting_set[j];
            }

            KDL::Add(*init_params.joint_ff_torques, *temp_torques, *new_ff_torques);
...
public:
    Friction_enabled_vereshchagin (Initialization parameters, std::vector<double> friction_tau) {
...
{
    //Try to make it iteractive....user to select given options
    if (argc < 6) {
    ...
    Friction_enabled_vereshchagin solver(init_params, friction_tau);
    solver.cart_to_jnt();
	return 0;
}
